...
// in the future we may only want to only do it on a date change
  let budget = getters.getBudgetByDate(data.date);
  if (!budget) throw new Error('Could not find a budget for the date ' + data.date);
  data.budget = budget.id;

// tell the budget category that the transaction is occuring so it can update its amount
  let budgetCategory = getters.getBudgetCategoryByBudgetAndCategory(budget.id, data.category);
  if (!budgetCategory) throw new Error('Could not find a budget cateogry for ' + data.category);
...
export const createTransaction = ({ commit, dispatch, getters }, data) => {
  ...
  dispatch('updateAccountBalance', {
    account: getters.getAccoundById(data.account),
    amount: transaction.amount
  });
...
export const loadBusinsesses = ({ state, commit }) => {
  // loads businsesses if they're not already loaded
